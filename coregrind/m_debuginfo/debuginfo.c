...
static const RUNTIME_FUNCTION *find_runtime_function(const DebugInfo *di, Addr rip)
{
    Word hi, lo, mid;
                              ...
                              Addr max_accessible )
{
    ...
    while(1)
    {
        ...
        if (uinfo->Version != 1)
        {
            VG_(printf)("unknown unwind info version %u at %p", uinfo->Version, uinfo);
            return False;
        }

        if (rip >= base + rtf->BeginAddress && rip < base + rtf->BeginAddress + uinfo->SizeOfProlog)
        ...
        for (i = 0; i < uinfo->CountOfCodes; i += get_code_size(&uinfo->UnwindCode[i]) )
        {
        ...
        rtf = (RUNTIME_FUNCTION *)&(uinfo->UnwindCode[(uinfo->CountOfCodes+1) & ~1]);
    }
